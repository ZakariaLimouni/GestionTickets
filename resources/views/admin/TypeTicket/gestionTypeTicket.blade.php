        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-white-800 overflow-hidden shadow-sm sm:rounded-lg" style="width: 90%;margin-left:70px">
                <div class="p-6 text-white-900 dark:text-white-100" >
                    <div class="container mx-auto px-4">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-lg font-semibold"
                                style="display: inline-block; border-bottom: 3px solid #006622; padding-left: 8px; padding-right: 8px;">
                                Liste des Type Tickets</h2>
                            <button id="createTypeTicketButton" class="add-TypeTicket-button toggle-element">
                            ...
                            <tbody>
                                @foreach ($TypeTickets as $TypeTicket)
                                    <tr class="{{ $loop->iteration % 2 == 0 ? 'even' : 'odd' }}">
                                        <td style="width: 40%">{{ $TypeTicket->libelle }}</td>
                                        <td >{{ $TypeTicket->status }}</td>
                                        <td class="table-buttons">
                                            <div style="display: flex; align-items: center;">
                                                <i id="modifyButton_{{ $TypeTicket->id }}" class="fas fa-pencil-alt icon-modify"
                                                    onclick="toggleEditForm('{{ $TypeTicket->id }}')"
                                                ...
                                                <form action="{{ route('admin.deleteTypeTicket', $TypeTicket->id) }}"
                                    ...
                                    <tr id="editTypeTicketFormRow_{{ $TypeTicket->id }}" style="display: none;">
                                        <form id="editTypeTicketForm_{{ $TypeTicket->id }}" method="POST"
                                            action="{{ route('admin.updateTypeTicket', $TypeTicket->id) }}">
                                            @csrf
                                            @method('PUT')
                                            <td>
                                                <div>
                                                    <x-text-input id="libelle" class="block mt-1 w-full"
                                                        type="text" name="libelle" value="{{ $TypeTicket->libelle }}"
                                            ...
                                            <td>
                                                <div class="flex items-center justify-center">
                                                    <select id="status" name="status"
                                                        ...
                                                        <option value="active"
                                                            {{ $TypeTicket->status == 'active' ? 'selected' : '' }}>Active
                                                        </option>
                                                        <option value="inactive"
                                                            {{ $TypeTicket->status == 'inactive' ? 'selected' : '' }}>
                                                            Inactive
                                                        </option>
                                                    </select>
                                            ...
                                            <td>
                                                <div class="flex items-center justify-center">
                                                    <x-primary-button id="cancelButton_{{ $TypeTicket->id }}"
                                ...
                                @endforeach
                                <tr id="createTypeTicketFormRow" style="display: none;">
                                    <form id="createTypeTicketForm" method="POST"
                                        action="{{ route('ajouterTypeTicket.store') }}">
                                        @csrf
                                        <td>
                                            <div>
                                        ...
                                        <td>
                                            <div class="flex items-center justify-center">
                                                <x-primary-button id="cancelCreateTypeTicketButton"
                    ...
                    <script>
                        const toggleElement = document.querySelector('#createTypeTicketButton .toggle-element');

                        document.getElementById('createTypeTicketButton').addEventListener('click', function() {
                            const formRow = document.getElementById('createTypeTicketFormRow');
                            var addButton = document.getElementById('createTypeTicketButton');
                        ...
                        function toggleCreateForm() {
                            var formRow = document.getElementById('createTypeTicketFormRow');
                            var addButton = document.getElementById('createTypeTicketButton');
                        ...
                        function toggleEditForm(TypeTicketId) {
                            var editFormRow = document.getElementById('editTypeTicketFormRow_' + TypeTicketId);
                            var modifyButton = document.getElementById('modifyButton_' + TypeTicketId);
                            var cancelButton = document.getElementById('cancelButton_' + TypeTicketId);
                        ...
                        function confirmDelete() {
                            return confirm("Êtes-vous sûr de vouloir supprimer cette Type de Ticket?");
                        }
                    </script>
